    ...
    runtime.block_on(async {
        ...
        if actors.is_empty() {
            if json {
                ...
                println!("{}", formatting::format_info("No actors are currently running."));
            }
            return Ok(());
        }
        ...
        for actor_id in &actors {
            ...
            nodes.insert(actor_id.clone(), ActorNode {
                id: actor_id.clone(),
                status,
                children: Vec::new(),
                parent: None,
        ...
        if json {
            ...
            // Output the result
            println!("{}", serde_json::to_string_pretty(&json!({
                "actors": actors.iter().map(|a| a.to_string()).collect::<Vec<_>>(),
                "tree": tree_json
            }))?);
...
fn print_tree(
    ...
    if let Some(node) = nodes.get(current_id) {
        ...
        for (i, child_id) in children.iter().enumerate() {
            ...
            print_tree(nodes, child_id, &child_prefix, is_last_child, max_depth, current_depth + 1);
        }
    }
}

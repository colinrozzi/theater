...
pub fn execute(args: &ShellArgs, _verbose: bool, _json: bool) -> Result<()> {
    ...
    println!(
        "{}",
        formatting::format_section("THEATER INTERACTIVE SHELL")
    );
    ...
    let mut client = match client_result {
        ...
        Err(e) => {
            eprintln!(
                "{}",
                formatting::format_error(&format!("Failed to connect to server: {}", e))
            );
    ...
    while running.load(Ordering::SeqCst) {
        ...
        // Add to history
        if !input.is_empty()
            && (history.is_empty() || history.back().map_or(true, |last| last != input))
        {
        ...
        match command.command.as_str() {
            ...
            _ => {
                println!(
                    "{}",
                    formatting::format_error(&format!("Unknown command: {}", command.command))
                );
...
fn handle_list_command(
    ...
    let actors = runtime.block_on(async { client.list_actors().await })?;
...
fn handle_inspect_command(
    ...
    // Get actor status
    let status = runtime.block_on(async { client.get_actor_status(actor_id.clone()).await })?;
    ...
    println!(
        "{}",
        formatting::format_key_value("ID", &formatting::format_id(&actor_id))
    );
    println!(
        "{}",
...
fn handle_state_command(
    ...
    // Get actor state
    let state = runtime.block_on(async { client.get_actor_state(actor_id.clone()).await })?;
...
fn handle_events_command(
    ...
    // Get actor events
    let events = runtime.block_on(async { client.get_actor_events(actor_id.clone()).await })?;
...
fn handle_start_command(
    ...
    // Start the actor
    let actor_id = runtime.block_on(async { client.start_actor(manifest_content, None).await })?;

    println!(
        "{}",
        formatting::format_success(&format!(
            "Actor started: {}",
            formatting::format_id(&actor_id)
        ))
    );
...
fn handle_stop_command(
    ...
    // Stop the actor
    runtime.block_on(async { client.stop_actor(actor_id.clone()).await })?;
...
fn handle_restart_command(
    ...
    // Restart the actor
    runtime.block_on(async { client.restart_actor(actor_id.clone()).await })?;
...
fn handle_message_command(
    ...
    runtime.block_on(async {
        client
            .send_actor_message(actor_id.clone(), message_data)
            .await
    })?;

...
pub fn execute(args: &SubscribeArgs, _verbose: bool, json: bool) -> Result<()> {
    ...
    runtime.block_on(async {
        println!("{} Subscribing to events for actor: {}", 
            style("ℹ").blue().bold(),
            style(actor_id.to_string()).cyan());
        
        if let Some(event_type) = &args.event_type {
            println!("{} Filtering events by type: {}", 
                style("ℹ").blue().bold(),
        ...
        loop {
            ...
            let response = match time::timeout(Duration::from_secs(1), client.receive_response()).await {
        ...
        if let Err(e) = client.unsubscribe_from_actor(actor_id, subscription_id).await {

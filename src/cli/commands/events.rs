    ...
    let content = match format {
        ...
        "csv" => {
            ...
            for event in events {
                ...
                let parent_hash_hex = event.parent_hash
                    .as_ref()
                    .map(|h| hex::encode(h))
...
fn display_events_csv(events: &[ChainEvent]) -> Result<()> {
    ...
    for event in events {
        ...
        let parent_hash = event.parent_hash
            .as_ref()
            .map(|h| hex::encode(h))
            .unwrap_or_else(|| String::from(""));
...
fn display_events_pretty(events: &[ChainEvent], actor_id: &TheaterId, detailed: bool, limit: usize) -> Result<()> {
    println!("{} Events for actor: {}", 
        style("ℹ").blue().bold(),
        style(actor_id.to_string()).cyan());
    ...
    for (i, event) in events.iter().enumerate() {
        ...
        if detailed {
            ...
            if let Ok(text) = std::str::from_utf8(&event.data) {
                if text.len() > 60 {
                    println!("  Data: {}... ({} bytes total)", &text[0..57], event.data.len());

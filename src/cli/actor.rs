...
pub async fn handle_actor_command(args: ActorArgs) -> Result<()> {
    match &args.command {
        ActorCommands::Start { manifest } => {
            start_actor(manifest.clone(), &args.address).await
        },
        ActorCommands::Stop { id } => {
            stop_actor(id.clone(), &args.address).await
        },
        ActorCommands::List { detailed } => {
            list_actors(*detailed, &args.address).await
        },
        ActorCommands::Subscribe { id } => {
            subscribe_to_actor(id.clone(), &args.address).await
        },
        ActorCommands::Inspect { id } => {
            inspect_actor(id.clone(), &args.address).await
        },
        ActorCommands::Restart { id } => {
            restart_actor(id.clone(), &args.address).await
        },
        ...
        ActorCommands::Call { id, method, payload, payload_file } => {
            call_actor(
                id.clone(), 
                method, 
                payload.clone(), 
...
async fn select_actor(id_opt: Option<String>, address: &str) -> Result<TheaterId> {
    match id_opt {
        ...
        None => {
            ...
            let actor_strings: Vec<String> = actors
                .iter()
                .map(|id| id.to_string())
                .collect();
...
async fn start_actor(manifest: Option<PathBuf>, address: &str) -> Result<()> {
    super::legacy::execute_command(
        super::legacy::Commands::Start { manifest },
        address
    ).await
}
...
async fn list_actors(detailed: bool, address: &str) -> Result<()> {
    super::legacy::execute_command(
        super::legacy::Commands::List { detailed },
        address
    ).await
}
...
async fn subscribe_to_actor(id: Option<String>, address: &str) -> Result<()> {
    super::legacy::execute_command(
        super::legacy::Commands::Subscribe { id },
        address
    ).await
}
...
async fn inspect_actor(id_opt: Option<String>, address: &str) -> Result<()> {
    ...
    println!("{} Fetching details for actor {}", 
    ...
    if let Some(Ok(data)) = framed.next().await {
        ...
        match response {
            ...
            _ => {
                println!("{} Unexpected response from server", style("Error:").red().bold());
            }
        }
    } else {
        println!("{} No response received from server", style("Error:").red().bold());
    }
...
async fn restart_actor(id_opt: Option<String>, address: &str) -> Result<()> {
    ...
    println!("{} To restart actor {}, we need the manifest (not yet implemented)", 
        style("Note:").yellow().bold(),
        style(actor_id.clone()).green() // Fixed: Added .clone()
    );
    
    println!("{} For now, we can only stop the actor", 
        style("Note:").yellow().bold()
    );
    
    // Stop actor
    let command = ManagementCommand::StopActor { id: actor_id.clone() }; // Fixed: Added .clone()
    let cmd_bytes = serde_json::to_vec(&command)?;
    framed.send(Bytes::from(cmd_bytes)).await?;
...
async fn view_actor_logs(id_opt: Option<String>, lines: usize, follow: bool, address: &str) -> Result<()> {
    println!("{}", style("Theater Actor Logs").bold().cyan());
    
    let actor_id = select_actor(id_opt, address).await?;
    
    println!("{} This feature requires server-side support for actor logs", 
        style("Note:").yellow().bold()
    );
    
    println!("Would fetch last {} lines of logs for actor {}", 
        lines,
    ...
    println!("[{}] {} State updated: version=2", 
        style("2023-02-26 12:35:01").dim(),
        style("INFO").blue().bold()
    );
    println!("[{}] {} Received message type='test'", 
...
async fn view_actor_state(
    ...
    println!("{} This feature requires server-side support for accessing actor state", 
        style("Note:").yellow().bold()
    );
    
    println!("Would fetch state for actor {} in {} format", 
        style(actor_id).green(),
        style(format).yellow()
    );
...
async fn call_actor(
    ...
    let payload_data = match (payload, payload_file) {
        ...
        (None, Some(path)) => {
            if !path.exists() {
                return Err(anyhow::anyhow!("Payload file not found: {}", path.display()));
            }
    ...
    println!("{} This feature requires server-side support for calling actor methods", 
        style("Note:").yellow().bold()
    );
    
    println!("Would call method {} on actor {} with payload:", 
        style(method).yellow(),
        style(actor_id).green()
    );

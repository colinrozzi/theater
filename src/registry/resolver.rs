...
pub fn resolve_actor_reference(reference: &str, registry_path: Option<&Path>) -> Result<(PathBuf, PathBuf)> {
...
fn resolve_direct_actor_path(path: &Path) -> Result<(PathBuf, PathBuf)> {
    if !path.exists() {
        return Err(Error::NotFound(format!("Actor manifest not found: {:?}", path)));
    }
    ...
    let component_path = if let Some(component) = manifest.get("component_path") {
        if let Some(component_str) = component.as_str() {
            ...
            return Err(Error::InvalidFormat("component_path is not a string".to_string()));
        }
    } else {
        return Err(Error::InvalidFormat("component_path not found in manifest".to_string()));
    };
...
fn resolve_registry_actor(reference: &str, registry_path: &Path) -> Result<(PathBuf, PathBuf)> {
    ...
    let actors = index.get("actors")
        .and_then(|a| a.as_array())
        .ok_or_else(|| Error::InvalidFormat("actors list not found in registry index".to_string()))?;
    
    let actor = actors.iter()
    ...
    let version_to_use = match version {
        Some(v) => {
            ...
            let versions = actor.get("versions")
                .and_then(|v| v.as_array())
                .ok_or_else(|| Error::InvalidFormat("versions list not found for actor".to_string()))?;
            
            let version_exists = versions.iter()
                .any(|ver| ver.as_str() == Some(&v));
            
            if !version_exists {
                return Err(Error::NotFound(format!("Version '{}' not found for actor '{}'", v, name)));
            }
        ...
        None => {
            ...
            actor.get("latest")
                .and_then(|l| l.as_str())
                .ok_or_else(|| Error::InvalidFormat("latest version not found for actor".to_string()))?
                .to_string()
        }
    };
    ...
    if !manifest_path.exists() {
        return Err(Error::NotFound(format!("Manifest not found: {:?}", manifest_path)));
    }
    
    if !component_path.exists() {
        return Err(Error::NotFound(format!("Component not found: {:?}", component_path)));
    }
    
    debug!("Resolved actor '{}:{}' to manifest {:?} and component {:?}",

//! Integration tests for the Runtime Handler
//!
//! This test creates a minimal Theater runtime with the runtime handler
//! and supporting handlers, spawns a test actor, and verifies the event chain.
...
fn get_test_wasm_path() -> PathBuf {
    ...
    manifest_dir.join("test-actors/runtime-test/target/wasm32-wasip1/release/runtime_test.wasm")
}
...
fn create_test_handler_registry(
    ...
    // Runtime handler - the one we're testing
...
#[tokio::test]
async fn test_runtime_handler_with_test_actor() -> Result<()> {
    // Skip test if WASM file doesn't exist (not built yet)
    let wasm_path = get_test_wasm_path();
    if !wasm_path.exists() {
        eprintln!(
            "Skipping test: WASM file not found at {:?}. Build with: cd test-actors/runtime-test && cargo component build --release",
            wasm_path
        );
        return Ok(());
    }
    ...
    match chain_result {
        Ok(Ok(Ok(events))) => {
            ...
            // Verify we have runtime-related events in the chain
            let runtime_events: Vec<_> = events
                .iter()
                .filter(|e| {
                    e.event_type.contains("runtime")
                        || e.event_type.contains("Runtime")
                        || e.event_type.contains("theater:simple/runtime")
                })
                .collect();

            println!("Found {} runtime-related events", runtime_events.len());

            // We should have runtime setup events and log events
            assert!(
                !runtime_events.is_empty(),
                "Expected runtime events in the chain"
            );
        }
        Ok(Ok(Err(e))) => {
    ...
    match state_result {
        Ok(Ok(Ok(Some(state)))) => {
            ...
            assert!(
                state_str.contains("success") || state_str.contains("passed"),
...
#[tokio::test]
async fn test_runtime_handler_event_recording() -> Result<()> {
    // Placeholder for more granular unit tests
    Ok(())
}
...
async fn test_handler_matching_debug() -> Result<()> {
    ...
    println!("Runtime handler imports: {:?}", runtime_imports);
    ...
    println!("IO handler imports: {:?}", io_imports);
    ...
    println!("Timing handler imports: {:?}", timing_imports);
    ...
    println!("Filesystem handler imports: {:?}", filesystem_imports);
    ...
    // Check export matches for runtime handler
    println!("\nHandler export matches:");
    if let Some(re) = &runtime_exports {
        let export_matches: Vec<_> = re.iter().filter(|exp| exports.contains(exp)).collect();
        let export_non_matches: Vec<_> = re.iter().filter(|exp| !exports.contains(exp)).collect();
        ...
        let imports_match = runtime_imports
            .as_ref()
            .map_or(false, |hi| hi.iter().any(|i| imports.contains(i)));
        let exports_match = !export_matches.is_empty();
        println!(
            "    -> Would activate (imports_match || exports_match): {} || {} = {}",
            imports_match,
            exports_match,
            imports_match || exports_match
        );
    }

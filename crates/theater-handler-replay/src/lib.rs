//! # Replay Handler
//!
//! Provides replay functionality for WebAssembly actors in the Theater system.
//! This handler replays actors from recorded event chains, returning the same
//! outputs that were originally produced.
//!
//! ## How It Works
//!
//! 1. Given a chain of events from a previous run, the ReplayHandler knows what
//!    host function calls were made and what they returned.
//!
//! 2. When `setup_host_functions` is called, it registers stub functions for
//!    all imported interfaces that return the recorded outputs.
//!
//! 3. As the component runs, each event is compared against the expected chain.
//!    Hash mismatches indicate divergence from the original execution.
//!
//! ## Usage
//!
//! ```ignore
//! // Load the expected chain
//! let expected_chain = load_chain_from_file("actor_chain.json")?;
//!
...
/// Shared state for tracking replay position
#[derive(Clone)]
pub struct ReplayState {
    ...
    /// List of interfaces we're replaying
...
impl ReplayState {
    ...
    /// Verify that an actual hash matches the expected hash
    pub fn verify_hash(&self, actual_hash: &[u8]) -> Result<(), String> {
        let pos = self.current_position();
        let expected = self.events.get(pos)
...
/// Handler for replaying actors from recorded event chains
#[derive(Clone)]
    ...
    E: EventPayload + Clone,
{
    ...
    fn start(
        ...
        Box::pin(async move {
            ...
            info!("Replay handler shut down. Progress: {}/{} events", current, total);
        ...
        for (import_name, import_item) in component_type.imports(engine) {
            ...
            if let ComponentItem::ComponentInstance(instance_type) = import_item {
                ...
                let mut interface = match actor_component.linker.instance(&import_name) {
                    Ok(i) => i,
                    Err(e) => {
                        warn!("Could not create linker instance for {}: {}", import_name, e);
                        continue;
                    }
                };
                ...
                for (func_name, export_item) in instance_type.exports(engine) {
                    if let ComponentItem::ComponentFunc(_func_type) = export_item {
                        ...
                        interface.func_new_async(
                            &func_name,
                            move |_ctx: StoreContextMut<'_, ActorStore<E>>, _params, results| {
                                ...
                                Box::new(async move {
                                    ...
                                    if let Some(expected) = state.current_event() {
                                        ...
                                        // In replay mode, we don't record events - we just
                                        // return the recorded outputs. The chain already exists.
                                        //
                                        // TODO: Parse the output bytes from expected.data and
                                        // convert to proper Val types for the results.

                                        // Advance to next event
                                        state.advance();
                                    } else {
                                        warn!("[REPLAY] No more expected events for {}", full_name);
                                    }

                                    // Fill results with defaults
                                    // In a full implementation, we'd deserialize from chain
    ...
    fn imports(&self) -> Option<Vec<String>> {
        ...
        // This allows the replay handler to satisfy any interface that was used
...
mod tests {
    ...
    fn test_replay_handler_creation() {
        ...
        let handler = ReplayHandler::new(events.clone());

        // Test the ReplayHandler directly (not through Handler trait)
        ...
        // Test interfaces discovered from chain
        let interfaces = handler.state().interfaces();
        assert!(interfaces.contains(&"theater:simple/runtime".to_string()));
    }
}

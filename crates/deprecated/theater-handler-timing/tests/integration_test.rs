//! Integration tests for the WASI Timing Handler
//!
//! This test creates a minimal Theater runtime with the timing handler
...
fn get_test_actor_dir() -> PathBuf {
    ...
    manifest_dir.join("test-actors/wasi-clocks-test")
}
...
fn get_test_wasm_path() -> PathBuf {
    ...
    get_test_actor_dir().join("target/wasm32-wasip1/release/wasi_clocks_test.wasm")
}
...
fn create_test_manifest_content() -> String {
    ...
    format!(
        r#"name = "wasi-clocks-test"
version = "0.1.0"
component = "{}"
description = "WASI clocks interface test actor"
save_chain = true
...
fn create_test_handler_registry(
    ...
    // Timing handler - the one we're testing
    ...
    // IO handler - provides wasi:io interfaces
...
#[tokio::test]
async fn test_timing_handler_with_test_actor() -> Result<()> {
    ...
    let spawn_result = timeout(Duration::from_secs(30), response_rx).await??;
        ...
        match timeout(Duration::from_millis(500), event_rx.recv()).await {
            ...
            Err(_) => {
                ...
                if last_event_time.elapsed() > Duration::from_secs(10) {
                    println!("No events for 10 seconds, stopping collection");
                    break;
                }
            }
        }
    ...
    match chain_result {
        Ok(Ok(Ok(events))) => {
            ...
            // Verify we have timing-related events in the chain
            let timing_events: Vec<_> = events
                .iter()
                .filter(|e| e.event_type.contains("timing") || e.event_type.contains("Timing"))
                .collect();

            println!("Found {} timing-related events", timing_events.len());

            // The test actor uses monotonic clock functions
            // We should see timing setup events at minimum
            assert!(
                !events.is_empty(),
    ...
    match state_result {
        Ok(Ok(Ok(Some(state)))) => {
            ...
            assert!(
                state_str.contains("WASI clocks") || state_str.contains("passed"),
...
#[tokio::test]
async fn test_timing_handler_matching_debug() -> Result<()> {
    // Skip test if WASM file doesn't exist (not built yet)
    let wasm_path = get_test_wasm_path();
    if !wasm_path.exists() {
        eprintln!(

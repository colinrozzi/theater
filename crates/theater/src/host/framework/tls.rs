...
pub fn load_certs(path: &str) -> Result<Vec<CertificateDer<'static>>> {
    ...
    if !path.exists() {
        return Err(anyhow::anyhow!("Certificate file not found: {}", path.display()));
    }
    
    if !path.is_file() {
        return Err(anyhow::anyhow!("Certificate path is not a file: {}", path.display()));
    }
    ...
    if certs.is_empty() {
        return Err(anyhow::anyhow!("No certificates found in file: {}", path.display()));
    }
...
pub fn load_private_key(path: &str) -> Result<PrivateKeyDer<'static>> {
    ...
    if !path.exists() {
        return Err(anyhow::anyhow!("Private key file not found: {}", path.display()));
    }
    
    if !path.is_file() {
        return Err(anyhow::anyhow!("Private key path is not a file: {}", path.display()));
    }
...
pub fn create_tls_config(cert_path: &str, key_path: &str) -> Result<RustlsConfig> {
    debug!("Creating TLS configuration with cert: {}, key: {}", cert_path, key_path);
...
pub fn validate_tls_config(cert_path: &str, key_path: &str) -> Result<()> {
    ...
    if !key_path.exists() {
        return Err(anyhow::anyhow!("Private key file not found: {}", key_path.display()));
    }
    
    if !cert_path.is_file() {
        return Err(anyhow::anyhow!("Certificate path is not a file: {}", cert_path.display()));
    }
    
    if !key_path.is_file() {
        return Err(anyhow::anyhow!("Private key path is not a file: {}", key_path.display()));
    }

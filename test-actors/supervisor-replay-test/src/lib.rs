//! Runtime-control replay test actor for Pack runtime.
//!
//! A deterministic actor that exercises the runtime control host functions:
//! - Imports `theater:simple/self.log`
//! - Imports `theater:simple/runtime.{spawn, list-actors, stop-actor}`
//! - Exports `theater:simple/actor.init`
//! - Exports `theater:simple/message-server-client.handle-send`
//! - Exports `theater:simple/lifecycle-handlers.handle-lifecycle-event`
//!
//! Commands (sent as message bytes in handle-send):
//! - `"spawn:<manifest_path>"` → spawn a child, store child_id in state
//! - `"list"` → list actors, log the count
...
// The runtime ops return `result<T, runtime-error>`; import them raw and
...
/// Render a `runtime-error` variant Value as its case name.
